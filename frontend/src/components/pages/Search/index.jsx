...
import { 
  Input, Card, Button, Typography, Empty, message, 
  Spin, Image, Flex, AutoComplete, Badge, Descriptions, 
  Tag, Row, Col 
} from 'antd';
import { 
  SearchOutlined, DownloadOutlined, FileImageOutlined, 
  PictureOutlined, UserOutlined 
      ...
      .map(e => ({
        ...
        label: (
          <Flex justify="space-between">
            <span>{e.employee_id} - <b>{e.employee_name}</b></span>
            <Text type="secondary" style={{ fontSize: 12 }}>{e.employee_department}</Text>
          </Flex>
        ),
      }));
  }, [searchId, employees]);
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '20px 10px' }}>
      ...
      <Flex vertical align="center" style={{ marginBottom: 40 }}>
        <Badge count={<PictureOutlined style={{ color: '#1890ff', fontSize: 20 }} />}>
          <Title level={2} style={{ margin: 0 }}>Employee Directory</Title>
        </Badge>
      ...
      >
        <AutoComplete
          ...
          style={{ width: '100%' }}
          options={options}
          onSelect={(val) => { setSearchId(val); handleSearch(val); }}
      ...
      {loading ? (
        ...
        <Flex vertical align="center" justify="center" style={{ padding: 100 }}>
          <Spin size="large" />
          <Text type="secondary" style={{ marginTop: 16 }}>Đang truy xuất dữ liệu...</Text>
        </Flex>
      ) : (
        (imageUrl || imageError) ? (
          <Row gutter={[24, 24]}>
            <Col xs={24} md={10}>
              <Card variant="borderless" style={{ textAlign: 'center', borderRadius: 16, overflow: 'hidden', background: '#fafafa' }}>
                {imageError ? (
                  <Empty description="Không có ảnh chân dung" />
                ) : (
                  <Image
                    src={imageUrl}
                    style={{ borderRadius: 8, width: '100%', maxWidth: 280, boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
                    placeholder={<Spin />}
                  />
                )}
              </Card>
            ...
            <Col xs={24} md={14}>
              <Card 
                title={<Flex gap={8} align="center"><UserOutlined /> Hồ sơ chi tiết</Flex>}
                variant="borderless" 
                style={{ borderRadius: 16, boxShadow: '0 8px 24px rgba(0,0,0,0.05)' }}
              >
                {selectedEmp ? (
                  <Descriptions column={1} bordered size="small">
                    <Descriptions.Item label="Họ tên"><Text strong>{selectedEmp.employee_name}</Text></Descriptions.Item>
                    <Descriptions.Item label="Mã NV">{selectedEmp.employee_id}</Descriptions.Item>
                    <Descriptions.Item label="Mã thẻ cũ">{selectedEmp.employee_old_id || '-'}</Descriptions.Item>
                    <Descriptions.Item label="Bộ phận">{selectedEmp.employee_department}</Descriptions.Item>
