...
const Dashboard = () => {
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      <Title level={3} style={{ marginBottom: 24 }}>System Overview</Title>

      {/* --- PHẦN 1: KEY METRICS --- */}
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        <Col xs={24} sm={8}>
            <Card variant="borderless"  style={{ height: '100%', background: '#1890ff', color: '#fff' }}>
                <Statistic title={<span style={{color:'rgba(255,255,255,0.8)'}}>Total Assets</span>} value={assets.length} valueStyle={{color:'#fff'}} prefix={<DatabaseOutlined />} />
                <div style={{ marginTop: 8, opacity: 0.8 }}>System managed devices</div>
            </Card>
        </Col>
        <Col xs={24} sm={8}>
            <Card variant="borderless"  style={{ height: '100%' }}>
                <Statistic title="In Use Ratio" value={Math.round((statusStats.inUse / total) * 100)} suffix="%" valueStyle={{ color: '#3f8600' }} prefix={<CheckCircleOutlined />} />
                <Progress percent={Math.round((statusStats.inUse / total) * 100)} showInfo={false} strokeColor="#3f8600" size="small" />
                <div style={{ marginTop: 8, fontSize: 12, color: '#8c8c8c' }}>{statusStats.inUse} devices currently assigned</div>
            </Card>
        </Col>
        <Col xs={24} sm={8}>
            <Card variant="borderless"  style={{ height: '100%' }}>
                <Statistic title="Broken / Critical" value={statusStats.broken + criticalAssets.length} valueStyle={{ color: '#cf1322' }} prefix={<WarningOutlined />} />
                <Progress percent={Math.round(((statusStats.broken + criticalAssets.length) / total) * 100)} showInfo={false} strokeColor="#cf1322" size="small" />
      ...
      <Row gutter={[24, 24]}>
                        ...
                        <List.Item>
                            <div style={{ width: '100%' }}>
                                <Flex justify="space-between" style={{ marginBottom: 4 }}>
                                    ...
                                    <Text type="secondary">{item.count} assets ({item.percent}%)</Text>
                                </Flex>
                                <Progress 
                                    percent={item.percent} 
                                    strokeColor={item.code === 'PC' ? '#1890ff' : (item.code === 'LPT' ? '#722ed1' : '#fa8c16')} 
                                    trailColor="#f5f5f5"
                                />
                            </div>
                        </List.Item>
        ...
        <Col xs={24} lg={8}>
            <Card 
                title={<Space><WarningOutlined style={{color: '#cf1322'}}/> Critical Health Attention</Space>} 
                variant="borderless" 
                style={{ minHeight: 400 }}
                bodyStyle={{ padding: 0 }}
            >
                {criticalAssets.length === 0 ? (
                    ...
                    <Table 
                        ...
                        size="small"
                        columns={[
                            { title: 'Code', dataIndex: 'asset_code', render: t => <Text strong>{t}</Text> },
                            { title: 'Model', dataIndex: 'model', ellipsis: true },
                            { title: 'Issue', dataIndex: 'notes', ellipsis: true, render: t => <Text type="secondary" style={{fontSize:11}}>{t || 'No notes'}</Text> },
                        ]}
                    />
                )}
            </Card>
