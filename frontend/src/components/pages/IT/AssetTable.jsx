...
const AssetTable = ({ 
  ...
  const columns = [
    ...
    // EMPLOYEE ID COLUMN
    {
      ...
      render: (id) => id ? (
        <Text strong style={{ fontSize: '14px',  color: '#262626' }} copyable={{ tooltips: false }}>
            {id}
        </Text>
    ...
    },
    {
      ...
      render: (u) => u ? (
        <Flex vertical gap={2}>
          <Text strong style={{ fontSize: '14px' }}>{u.employee_name}</Text>
          <Text type="secondary" style={{ fontSize: '12px' }}>{u.department}</Text>
        </Flex>
    ...
    // --- 5. STATUS & HEALTH ---
    {
      ...
      render: (status) => {
        ...
        if (status === 'In Use') { color = 'processing'; }
        else if (status === 'Spare') { color = 'success'; }
        else if (status === 'Broken') { color = 'error'; }
        
        return (
            <Tag color={color} style={{ fontSize: '11px', padding: '2px 8px', minWidth: '65px', textAlign: 'center' }}>
                {label.toUpperCase()}
            </Tag>
        );
      }
    },
    {
      ...
      render: (h) => {
        ...
        return (
          <Tag color={color} style={{ fontSize: '12px', width: '100%', textAlign: 'center', fontWeight: 600, border: 'none', color: '#fff' }}>
            {h}
          </Tag>
        );
      }
    ...
    { 
      ...
      width: 100, 
      align: 'center',
      render: (d) => <Text type="secondary" style={{ fontSize: '13px' }}>{d ? dayjs(d).format('DD/MM/YY') : '-'}</Text>
    },
