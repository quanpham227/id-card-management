...
const AssetStats = ({ assets = [], categories = [], activeFilter, onFilterChange }) => {
  ...
  const categoryStatsArray = Object.values(stats.catCounts).filter(
    (c) => c.count > 0 || c.code !== 'OTHER'
  );
  ...
  const statusItems = [
    ...
    },
    {
      title: 'Spare (Kho)',
      filterKey: 'Spare',
      value: stats.statusCounts.spare,
      icon: <InboxOutlined />,
      color: '#2f54eb',
      bg: '#f0f5ff',
    },
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <Text strong style={{ display: 'block', marginBottom: 8, fontSize: 12, color: '#8c8c8c' }}>
        BY CATEGORY
      </Text>
            ...
            >
              <Flex align="center" gap={12}>
                ...
                <Flex vertical>
                  ...
                  <Statistic
                    value={item.count}
                    valueStyle={{ fontSize: '18px', fontWeight: 700, lineHeight: 1 }}
                  />
                </Flex>
              </Flex>
            </Card>
