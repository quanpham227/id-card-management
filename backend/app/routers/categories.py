...
def create_category(category: schemas.CategoryCreate, db: Session = Depends(get_db)):
    ...
    if (
        db.query(models.AssetCategory)
        .filter(models.AssetCategory.code == category.code)
        .first()
    ):
    ...
    if (
        db.query(models.AssetCategory)
        .filter(models.AssetCategory.name == category.name)
        .first()
    ):
...
def update_category(
    ...
    db_cat = (
        db.query(models.AssetCategory)
        .filter(models.AssetCategory.id == category_id)
        .first()
    )
    if not db_cat:
    ...
    if category_update.code != db_cat.code:
        if (
            db.query(models.AssetCategory)
            .filter(models.AssetCategory.code == category_update.code)
            .first()
        ):
            raise HTTPException(status_code=400, detail="Category Code already exists")
...
def delete_category(category_id: int, db: Session = Depends(get_db)):
    ...
    linked_assets = (
        db.query(models.Asset).filter(models.Asset.category_id == category_id).first()
    )
    if linked_assets:
        raise HTTPException(
            status_code=400,
